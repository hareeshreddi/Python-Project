...
def sec_get_Attack_Data_Master_files(folder_type):
	...
	for path_to_folder in path_to_folders:
		...
		for dataset_file in list_of_files_in_folder:
			path_to_dataset_file = path_to_folder + dataset_file
			file_object = open(path_to_dataset_file, "r")
			list_for_return.append(file_object)
	return list_for_return

def sec_get_Training_Data_Master_files():
	path_to_folder = "ADFA-LD/Training_Data_Master/"
	list_of_files_in_folder = sorted(os.listdir(path_to_folder))
	list_for_return = []
	for dataset_file in list_of_files_in_folder:
...
with open("Adduser.txt","w") as x:
	for key in Sorted_List_Adduser[:int(0.3*len(Sorted_List_Adduser))]:
		x.write("%s       %d\n" % (str(key), dictionary_Adduser[key]))
	x.close()
with open("Adduser2.txt","w") as x:
	for key in Sorted_List_HydraFTP[:int(0.3*len(Sorted_List_HydraFTP))]:
		x.write("%s       %d\n" % (str(key), dictionary_HydraFTP[key]))
	x.close()
with open("Adduser3.txt","w") as x:
	for key in Sorted_List_HydraSSH[:int(0.3*len(Sorted_List_HydraSSH))]:
		x.write("%s       %d\n" % (str(key), dictionary_HydraSSH[key]))
	x.close()
with open("Adduser4.txt","w") as x:
	for key in Sorted_List_JavaMeterpreter[:int(0.3*len(Sorted_List_JavaMeterpreter))]:
		x.write("%s       %d\n" % (str(key), dictionary_JavaMeterpreter[key]))
	x.close()
with open("Adduser5.txt","w") as x:
	for key in Sorted_List_Meterpreter[:int(0.3*0.3*len(Sorted_List_Meterpreter))]:
		x.write("%s       %d\n" % (str(key), dictionary_Meterpreter[key]))
	x.close()
with open("Adduser6.txt","w") as x:
	for key in Sorted_List_WebShell[:int(0.3*len(Sorted_List_WebShell))]:
		x.write("%s       %d\n" % (str(key), dictionary_WebShell[key]))
	x.close()
with open("Adduser7.txt","w") as x:
	for key in Sorted_List_Training[:int(0.3*len(Sorted_List_Training))]:
		x.write("%s       %d\n" % (str(key), dictionary_Training[key]))
	x.close()
...
with open("Adduser_1.txt","w") as u:
	for key in dictionary_Adduser:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_Adduser[key]))
with open("Adduser_2.txt","w") as u:
	for key in dictionary_HydraFTP:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_HydraFTP[key]))
with open("Adduser_3.txt","w") as u:
	for key in dictionary_HydraSSH:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_HydraSSH[key]))
with open("Adduser_4.txt","w") as u:
	for key in dictionary_JavaMeterpreter:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_JavaMeterpreter[key]))
with open("Adduser_5.txt","w") as u:
	for key in dictionary_Meterpreter:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_Meterpreter[key]))
with open("Adduser_6.txt","w") as u:
	for key in dictionary_WebShell:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_WebShell[key]))
with open("Adduser_7.txt","w") as u:
	for key in dictionary_Training:
		if key in Sorted_List_All_Top30:
			u.write("%s                  %d\n" % (str(key), dictionary_Training[key]))
